...
export function showErrors(error, errorLog) {
	if (error == 'TypeError: Failed to fetch') {
		buildErrorLog(
			'red',
			'Por favor conectese a internet para cargar sus projectos',
			errorLog
		);
...
export function setListName(e, saveList) {
	...
	document.querySelector('.actual-list').children[0].classList.remove('hide');
...
export function disableCalendar(config) {
	const {
		containers: { calendarSurface, forward, backward },
		calendar: { dateBackward, date, dateForward },
	} = config;
...
export function copyText(e) {
	localStorage.setItem(
		'clipboard',
		e.parentNode.parentNode.children[1].textContent
	);
}
...
export function loadProject(surface, Project, main, listName, saveList) {
	//This is needed in the above one
