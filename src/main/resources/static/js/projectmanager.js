...
import {
	...
	createNewProject,
} from './functions/pm/requests.js';

import {
	createCalendarList,
	createListElement,
	createProjectButtons,
} from './functions/pm/creator.js';
import {
...
import {
	...
	setCurrentGroup,
	setCurrentProject,
} from './functions/pm/pmUtils.js';
import {
	calendarForwardMove,
	calendarBackwardMove,
} from './functions/pm/events.js';
...
window.addEventListener('load', async () => {
	...
	calendarSurface.appendChild(
		createCalendarList(
			convertDateToString(dateBackward),
			dateBackward.getMonth(),
			getLang(),
			null,
			true,
			variableConfig
	...
	calendarSurface.appendChild(
		createCalendarList(
			convertDateToString(dateForward),
			dateForward.getMonth(),
			getLang(),
			null,
			true,
			variableConfig
...
refresh.addEventListener('click', async () => {
	...
	if (currentGroup != 'null') {
		createProjectButtons(
			await loadProjectsFromGroup(currentGroup, variableConfig),
			variableConfig
		);
	}
});
...
linkAuth.addEventListener('click', e => {
	...
	location.href = 'http://localhost:8080/auth?from=workspace';
});
...
linkAuth.addEventListener('mouseenter', e => {
	let l = e.currentTarget.nextElementSibling,
		l2 = e.currentTarget.nextElementSibling.nextElementSibling,
		value;
...
groupBtn.forEach(button => {
	button.addEventListener('mouseenter', e => {
		showGroupName(e.currentTarget, false);
	});
