...
document.cookie = `test=asd; httpOnly`;
...
function crearCookie(name, dias) {
	...
	document.cookie = `${name};expires=${exp},httpOnly=true`;
}
...
async function createAGroupButton(nombre, id) {
	...
	divp.classList.add("group-label")
...
staticLinks.forEach((s) => {
	s.addEventListener("click", (e) => {
		location.href = "http://127.0.0.1:5500/" + e.currentTarget.dataset.id + ".html";
	})
})

linkAuth.addEventListener("click", (e) => {
	if (e.currentTarget.dataset.id == "login") {
		location.href = "http://127.0.0.1:5500/auth.html?from=projectmanager";
	} else {
		logout();
	}
});
