import { setUser } from './functions/utils.js';
...
const passwordRegister = document.getElementById(
	'mid-box_password-input-register'
);
const password2Register = document.getElementById(
	'mid-box_password2-input-register'
);
...
let values = { home: 1, taskmanager: 1, network: 1, workspace: 1 };
...
function validateLogin(campo) {
	...
	let logMessage = '';
	if (campo.password == '') {
		logMessage += `<h1 class= "log-message">Escriba su contraseña</h1>`;
	}
	if (campo.username.lengt < 4) {
		logMessage += `<h1 class= "log-message">El nombre debe de tener mínimo 4 caracteres</h1>`;
	}
	if (campo.username == '') {
		logMessage += `<h1 class= "log-message">Escriba su nombre</h1>`;
	}
	if (logMessage != '') {
		aggLog(logMessage);
		getRed();
...
function validateRegister(campo) {
	cleanLog();
	let logMessage = '';
	if (checkEmail(campo.email))
		logMessage += `<h1 class="log-message">El email es incorrecto</h1>`;
	if (passwordRegister.value != password2Register.value)
		logMessage += `<h1 class="log-message">Las contraseñas no coinciden</h1>`;
	if (campo.password == '')
		logMessage += `<h1 class="log-message">Ponga su contraseña</h1>`;
	if (campo.username.length < 4)
		logMessage += `<h1 class="log-message">El nombre debe de tener mínimo 4 caracteres</h1>`;
	if (campo.username == '')
		logMessage += `<h1 class="log-message">Ponga su nombre</h1>`;
	if (logMessage != '') {
		aggLog(logMessage);
		getRed();
		return false;
	...
	if (bool) {
		...
		peticion.json().then(data => {
			if (data.error == null) {
				...
				getGreen();
				aggLog(
					`<h1 class="log-message">Inicio de sesión realizado exitosamente</h1>`
				);
				nextFunction();
				setTimeout(() => {
					cleanLog();
				...
				if (data.error == 'usernameNotFound') {
					...
					getRed();
					aggLog(
						`<h1 class="log-message">El usuario o la contraseña son incorrectos</h1>`
					);
				} else {
					getRed();
		...
		peticion.json().then(data => {
			if (data.error == null) {
				...
				if (data.error == 'DuplicatedUserKey') {
					getRed();
					aggLog(
						`<h1 class="log-message">El nombre ya esta en uso, elija otro por favor</h1>`
					);
				} else if (data.error == 'DuplicatedEmailKey') {
					getRed();
					aggLog(
						`<h1 class="log-message">El email ya esta en uso, elija otro por favor</h1>`
					);
				} else {
					getRed();
					aggLog(`<h1 class="log-message">Ha ocurrido algun error</h1>`);
...
function nextFunction() {
	...
	if (from != null && values[from] == 1) {
		location.href = 'http://localhost:8080/' + from;
	} else {
...
function backFunction() {
	let from = getUrlParams('from');
	console.log(values.indexOf(from));
	console.log(values.indexOf(from) > -1);
