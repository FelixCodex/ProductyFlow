...
export function showModifiableMembers(peticion, box, gid, errorLog) {
	...
	if (peticion.error == null) {
		...
		for (let i = 1; i < ids.length; i++) {
			...
			box.appendChild(
				createModifiableMember(names[i], ids[i], permisses[i], gid, errorLog)
			);
		}
	}
}
...
export function createJoinDialog(div, dialogB, user, config) {
	...
	div.innerHTML = `
                ...
                <div class="join-p-box">
                    <p class="join-p">Join a group and discover the opportunities that working in a team offers.</p>
                    <p class="join-p">You can join a group by entering the group code</p>
                </div>`;
...
export function createCreateDialog(div, dialogB, user, groupBox, config) {
	...
	div.innerHTML = `
                ...
                <div class="create-p-box">
                    <p class="create-p">Create a group and share the opportunities that working in a team
							offers. The image must be less than 5MB.</p>
                </div>`;
...
export function createModifiableMember(
	memberName,
	id,
	memberPermission,
	gid,
...
export async function createGroupInfoDialog(
	adm,
	div,
	dialogB,
	groupName,
...
export function createInviteDialog(div, dialogB, groupId, errorLog) {
	...
	div.innerHTML = `
                ...
                <div class="dialog-p-box">
                    ...
                    <p class="dialog-p">You can invite someone by passing the user's name.</p>
                </div>
...
export async function createChangeGroupInfoDialog(
	div,
	dialogB,
	groupName,
	groupId,
	...
	changeButtonSave.addEventListener('click', async () => {
		...
		if (file) {
			...
			let group =
				getGroupButton(groupId).parentNode.lastElementChild.firstElementChild;
