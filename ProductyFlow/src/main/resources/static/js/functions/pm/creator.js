...
import { changeGroupInfoEvent, deleteEvent, groupInfoEvent, inviteEvent, leaveEvent, permissEvent } from "./events.js";
import { appearInput, checkTheBox, cleanSurface, createNoProjectsFoundElement, editProjectName, getActualGroup, getActualProject, hideGroupName, hideModifiButtons, loadCalendars, OnInput, openFirstProject, removeDisableModeToTheDateBtns, removeNoProjectFoundElement, setActualGroup, setActualProject, setIsSaveToFalse, setIsSaveToTrue, setValue, showGroupName, showModifiButtons, startBounceAnimation, updateDate } from "./pmUtils.js";
import { deleteProject, getGroupImg, loadCalendarList, loadListOnAction, loadProjectsFromGroup, update, updateCalendar } from "./requests.js";
import { convertStringDateToStringEs, convertStringDateToStringEn, monthsEn, monthsEs, copyTextToClipboard } from "../utils.js";
...
export async function groupButtonClickEvent(e, config) {
    ...
    const { containers: { projectsBox, surfaceList, calendarSurface, listName, errorLog, saveList, forward, backward }, calendar, userPermiss } = config;
...
export function createMemberOptions(menu, config) {
    cleanSurface(menu);
    menu.appendChild(createPopoupMenuOption("Group Info", groupInfoEvent, config))
    menu.appendChild(createPopoupMenuOption("Invite people", inviteEvent, config))
    menu.appendChild(createPopoupMenuOption("Leave group", leaveEvent, config))
}

export function createAdminOptions(menu, config) {
    cleanSurface(menu);
    menu.appendChild(createPopoupMenuOption("Group Info", groupInfoEvent, config, true))
    menu.appendChild(createPopoupMenuOption("Invite people", inviteEvent, config))
    menu.appendChild(createPopoupMenuOption("Change group info", changeGroupInfoEvent, config))
    menu.appendChild(createPopoupMenuOption("Permission Settings", permissEvent, config))
...
export async function createAGroupButton(nombre, id, config, bool) {
    const { containers: { groupBox, errorLog } } = config;
...
export function createCalendarList(date, month, lang, groupId, condition, config) {
    const { containers: { listName, errorLog, saveList } } = config;
...
export function createOneProjectButton(nombre, id, config) {
    const { containers: { saveList, projectsBox } } = config;
...
export function createModifiBtns(config) {
    const { containers: { saveList } } = config;
    const div = document.createElement("DIV");
...
export function createProjectButtons(Lists, config) {
    const { containers: { projectsBox } } = config;
    ...
    if (Lists && Lists.projects) {
        ...
        projectsBox.appendChild(createNoProjectsFoundElement("No hay ninguna lista"));
    }
